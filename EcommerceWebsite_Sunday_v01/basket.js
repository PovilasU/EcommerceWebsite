...
function loadBasket(){
    ...
    for(var i=0; i<basketArray.length; ++i){
		...
		customerIDs.push({customerID: basketArray[i].customerID, count: 1});//Add to product array
		
		// customerID: basketArray[i].customerID 
    }
    ...
    htmlStr += "<input type='hidden' name='prodIDs' value='" + JSON.stringify(prodIDs) + "'>";
        htmlStr += "<input type='hidden' name='customerIDs' value='" + JSON.stringify(customerIDs) + "'>";
...
function addToBasket(prodID, prodName, customerID){
    //Get basket from local storage or create one if it does not exist
    var basketArray;
    if(sessionStorage.basket === undefined || sessionStorage.basket === ""){
        basketArray = [];
    ...
    basketArray.push({id: prodID, name: prodName, customerID: customerID});
